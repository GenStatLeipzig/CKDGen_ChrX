#' ---
#' title: "Colocalisation Analysis (using Conditional Statistics): eGFR & Uric Acid: GTEx & NephQTL"
...
for (i in c(9, 21, 22)) { # Conditional Statistics only for Region 9, Region 21 and Region 22
  ...
  if (length(grep_ii) > 0) {
    ...
    matched                              = match(step10_lifted_filt_locus_i$ID_UKBB, cojo_cond_ii$SNP)
    step10_lifted_filt_locus_i$beta_cond = cojo_cond_ii$bC[matched]
    step10_lifted_filt_locus_i$se_cond   = cojo_cond_ii$bC_se[matched]
    step10_lifted_filt_locus_i$p_cond    = cojo_cond_ii$pC[matched]
  ...
  if (nrow(eqtl_j_filt_ij) > 0) {
    ...
    save(coloc_ij, file = paste0("../temp/07_coloc_results/", "07_coloc_cond_gtex_sub_", sub, "_i_", i, "_ii_", ii, "_j_", j, ".RData"))
  }
  }
}
}
...
for (i in c(9, 21, 22)) { # Conditional Statistics only for Region 9, Region 21 and Region 22
  ...
  if (length(grep_ii) > 0) {
    ...
    step10_filt_locus_i$p_cond    = cojo_cond_ii$pC[matched]
  ...
  if (nrow(neptune_j_filt_ij) > 0) {
    ...
    ### Save Colocalisation Analysis Results
    save(coloc_ij, file = paste0("../temp/07_coloc_results/", "07_coloc_cond_nephqtl_sub_", sub, "_i_", i, "_ii_", ii, "_j_", j, ".RData"))
  }
  }
}
}
...
for (j in 1 : length(eqtl_files)) {
  ...
  for (i in c(9, 21, 22)) {
    ...
    if (sum(grepl(pattern = paste0("07_coloc_cond_gtex_sub_", sub, "_i_", i, "_ii_", ii, "_j_", j, ".RData"),
                  res_files)) > 0) {

      load(paste0("../temp/07_coloc_results/", "07_coloc_cond_gtex_sub_", sub, "_i_", i, "_ii_", ii, "_j_", j, ".RData"))
      res_ij = coloc_ij
      rm(coloc_ij)
...
for (j in 1 : length(neptune_files)) {
  ...
  for (i in c(9, 21, 22)) {
    ...
    if (sum(grepl(pattern = paste0("07_coloc_cond_nephqtl_sub_", sub, "_i_", i, "_ii_", ii, "_j_", j, ".RData"),
                  res_files)) > 0) {

      load(paste0("../temp/07_coloc_results/", "07_coloc_cond_nephqtl_sub_", sub, "_i_", i, "_ii_", ii, "_j_", j, ".RData"))
      res_ij = coloc_ij
      rm(coloc_ij)
...
png(paste0("../results/07_coloc_cond_heatmap_", sub_names[sub], ".png"),
    width = width_sub[sub],
    height = height_sub[sub],
    res = 300)
# png(paste0("../results/07_coloc_cond_heatmap_reduced_", sub_names[sub], ".png"),
#     width = width_sub[sub],
#     height = height_sub[sub],
#     res = 300)
