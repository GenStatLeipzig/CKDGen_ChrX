...
setwd("/net/ifs1/san_projekte/projekte/genstat/02_projekte/1911_CKDgen_chrX/13_github/CKDGen_ChrX/scripts/")
...
#' Add direction
arrow.data          <- read_excel(path = "/net/ifs1/san_projekte/projekte/genstat/02_projekte/1911_CKDgen_chrX/14_submissions/2023-05-natcomm/revision/SupplementalTables_230929_noDescription.xlsx",
                                  sheet = "TableS15")
plotData8.sorted    <- plotData8[, c(1, 5, 7, 6, 2, 4, 3)]
arrow.matrix        <- plotData8.sorted[, c(2 : 7)]
arrow.matrix.pheno  <- c("eGFR_ALL",
                         "eGFR_MALE",
                         "eGFR_FEMALE",
                         "UA_ALL",
                         "UA_MALE",
                         "UA_FEMALE")
arrow.matrix.tissue <- c("Kidney_Cortex_Tubulointerstitial",
                         "Whole_Blood",
                         "Whole_Blood",
                         "Muscle_Skeletal",
                         "Kidney_Cortex_Tubulointerstitial",
                         "Thyroid",
                         "Stomach",
                         "Kidney_Cortex_Tubulointerstitial")
for (i in 1 : length(arrow.matrix.tissue)) {
  
  for (j in 1 : length(arrow.matrix.pheno)) {
    
    dummy <- ((arrow.data$CKD_pheno == arrow.matrix.pheno[j]) & (arrow.data$gene == plotData8$gene[i]) & (arrow.data$tissue == arrow.matrix.tissue[i]))
    
    if (sum(dummy) > 0) {
      
      arrow.matrix[i, j] <- arrow.data$sameEffectDir_CKD_eQTL[dummy]
    }
    else {
...
myLabels <- data.table(num = c(1 : (nrow(arrow.matrix) * ncol(arrow.matrix))),
                       pfeilnum = "")
for (i in 1 : nrow(arrow.matrix)) {
  
  for (j in 1 : ncol(arrow.matrix)) {
    
    if (!is.na(arrow.matrix[i ,j])) {
      
      if (arrow.matrix[i, j]) {
        
        myLabels$pfeilnum[(((i - 1) * ncol(arrow.matrix)) + j)] <- "\u2191\u2191"
      }
      else {
...
for (i in 1 : nrow(arrow.matrix)) {  # eGFR_MALE, eGFR_FEMALE
  
  for (j in c(2, 3)) {
    
    myLabels$pfeilnum[(((i - 1) * ncol(arrow.matrix)) + j)] <- myLabels$pfeilnum[(((i - 1) * ncol(arrow.matrix)) + 1)]
  }
}
for (i in c(3 : 5)) {  # UA_MALE, UA_FEMALE
  
  for (j in c(5, 6)) {
    
    myLabels$pfeilnum[(((i - 1) * ncol(arrow.matrix)) + j)] <- myLabels$pfeilnum[(((i - 1) * ncol(arrow.matrix)) + 4)]
  }
}

myPlot1 + geom_text(aes(label = myLabels$num))
myPlot1 + geom_text(aes(label = myLabels$pfeilnum), size = 7)
...
pdf_from_png(code2parseOrPlot = myPlot1 + geom_text(aes(label = myLabels$pfeilnum), size = 7), 
